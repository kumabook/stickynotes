...
function sendRequest(method, url, params) {
  ...
  req.onload = function() {
    logger.trace('response status: ' + req.status);
    logger.trace('---------------- response body ---------------');
    logger.trace(req.responseText);
    if (req.status >= 400) {
      if (req.status === 401) {
        logout();
      }
...
function login(email, password) {
  return fetchAccessToken(email, password).then((token) => {
    setAccessToken(token);
    return sendRequest('GET', config.rootUrl + '/api/v1/me.json').then((user) => {
      setUser(user);
      return user;
    });
  });
