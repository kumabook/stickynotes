  ...
  return stickynotes.DBHelper.getVersion(c).then((currentVersion) => {
    ...
    while (true) {
      if (currentVersion < targetVersion) {
        ...
        stickynotes.Logger.trace(`Execute ${migrations.length} migrations`);
...
stickynotes.DBHelper.fetch = function(Class, sql, params = {}) {
  ...
  let c;
  let items;
  stickynotes.Logger.trace(`Execute: ${sql} params: ${JSON.stringify(params)}`);
