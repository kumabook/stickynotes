...
stickynotes.Sticky.create = function(param) {
  ...
  console.log('create sticky ' + param.id);
  try {
    statement.execute();
    dbConn.asyncClose();
  } catch (e) {
 ...
 */
stickynotes.Sticky.prototype.setTags = function() {
  var dbConn = stickynotes.DAO.getDBConn();
  var statement = dbConn.createStatement(
  ...
  for (var i = 0; i < this.tagStrs.length; i++) {
    str = stickynotes.Tag.trimWhiteSpace(this.tagStrs[i]);
