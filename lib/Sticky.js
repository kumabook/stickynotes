...
stickynotes.Sticky.fetchByPage = function(page, key) {
  ...
  try {
    while (statement.executeStep()) {
      ...
      sticky.tags = sticky.getTags();
      if (!sticky.is_deleted && !key || sticky.filter(key)) {
        result.push(sticky);
      }
    }
  } catch (e) {
...
stickynotes.Sticky.fetchUpdatedStickiesSince = function(date) {
  ...
  var dbConn = stickynotes.DBHelper.getDBConn();
  //TODO: optimize  query, select tags together
  var sql = 'SELECT * FROM sticky WHERE' +
...
stickynotes.Sticky.create = function(_param) {
  var param = Object.create(_param);
  if (!param.id) {
  ...
  param.is_deleted = param.is_deleted ? 1 : 0;
...
stickynotes.Sticky.prototype.save = function() {
  ...
  keys.forEach(function(key) {
    if (key === 'is_deleted') {
      statement.params[key] = self[key] ? 1 : 0;
    } else {
...
stickynotes.Sticky.row2Obj = function(row, name_array) {
  if (!name_array)
    return {
      ...
      is_deleted: Boolean(parseInt(row.is_deleted))
    };
  else {
    var obj = { };
