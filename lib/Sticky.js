...
stickynotes.Sticky.count = function() {
  const sql = 'SELECT count(*) FROM sticky';
  const DBHelper = stickynotes.DBHelper;
  let c;
  return DBHelper.connection().then((conn) => {
    c = conn;
    return c.execute(sql, {});
  }).then(rs => rs[0].getResultByIndex(0))
    .then((result) => {
      c.close();
      return result;
    });
...
stickynotes.Sticky.fetchItems = function(offset, limit) {
  const sql = `SELECT * FROM sticky LIMIT ${limit} OFFSET ${offset}`;
  return this.fetch(sql).then((stickies) => {
    return Promise.all(stickies.map((sticky) => {
      return sticky.fetchTags().then(() => sticky.fetchPage());
    }));
