...
const sidebar = require('sdk/ui/sidebar').Sidebar({
  ...
  onAttach: function(worker) {
    ...
    worker.port.on('jump', function(sticky, url) {
...
pageMod.PageMod({
  ...
  onAttach: function(worker) {
    ...
    worker.port.on('delete', function(sticky) {
      ...
      _sticky.remove();
      worker.port.emit('delete-sticky', sticky);
...
var preferencePanel = panels.Panel({
  width: 460,
  height: 400,
  contentURL: 'chrome://stickynotes/content/preference.xul'
});
