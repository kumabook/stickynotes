...
var DAO  = 
	...
	dbConn.executeSimpleSQL("CREATE TABLE IF NOT EXISTS sticky(id INTEGER PRIMARY KEY , page_id INTEGER , left INTEGER, top INTEGER, width INTEGER, height INTEGER, content TEXT, color TEXT, tag TEXT)");
	dbConn.executeSimpleSQL("CREATE TABLE IF NOT EXISTS page(id INTEGER PRIMARY KEY , url TEXT UNIQUE, title TEXT)");
    },
    dropTables: function(){
	var dbConn = DAO.getDBConn();
	...
	var dbConn = DAO.getDBConn();
	var sql = "insert into sticky values('" + sticky.id + "','" + sticky.page_id + "','"  + sticky.left + "','" + sticky.top 
	    + "','" + sticky.width + "','" + sticky.height + "','" + sticky.content + "','" + sticky.color + "','" + sticky.tag + "')";
	try {
	...
	var sql = "UPDATE sticky SET id = '" + sticky.id + "', page_id = '" + sticky.page_id + "', left = '" + sticky.left + "', top = '" + sticky.top 
	    + "', width = '" + sticky.width + "', height = '" + sticky.height + "', content = '" + sticky.content
	    + "', color ='" + sticky.color + "', tag = '" + sticky.tag + "' WHERE id = '"+ sticky.id + "'";
	try{
	    dbConn.executeSimpleSQL(sql);
	} catch (x) {
...
//	page.page_id = Math.round(Math.random() * 10000);
	var sql = "insert into page values('" + page.id + "','"  + page.url + "','" + page.title + "')";
