...
//    this.change_size.addEventListener("mousedown",
	...
	"mousedown",
	    ...
	    if( ( right - Sticky.changeElemSize <  e.clientX  && 
		  e.clientX < right) 
		&&  (bottom - Sticky.changeElemSize <  e.clientY  &&  e.clientY < bottom)){
...
Sticky.prototype.addStickySidebar = function(){
    ...
    if(!url_item){
	url_item = Sidebar.createSidebarUrlItem({ id: this.id, url: this.url, title: this.title });
    }
...
Sticky.prototype.jump = function(){
	...
	window.parent.content.document.getElementById("sticky_id_" + this.id ).focus();
    else {
	...
	var focus_sticky = function (e) {
	    ...
	    window.parent.content.scrollTo(that.left - window.parent.content.innerWidth/2 , that.top - window.parent.content.innerHeight/3);
	    //        window.parent.content.document.getElementById("sticky_id_" + id).focus();
	    
	    var event = { notify: function(timer) { 
			      window.parent.content.document.getElementById("sticky_id_" + that.id ).focus();
			  }
			};
...
Sticky.prototype.toString = function(){
    return "sticky:" + this.id + ", " + this.page_id + ", " + this.left + ", " + this.top + ", " + 
	this.width + ", " + this.height + ", " + this.content + ", " + this.color +
	", " + this.tag;
};
